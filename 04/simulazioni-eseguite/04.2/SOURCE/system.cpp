...
void System :: initialize_velocities(){
  ...
  if(_restart){
    ...
    for (int i=0; i<_npart; i++){
      ...
      // per l'ESERCIZIO 4.2, ho modificato il codice affinché la distribuzione di velocità sia una delta
...
void System :: read_configuration(){
  ...
  if(cinf.is_open()){
    ...
    for(int i=0; i<_npart; i++){
      ...
      // per l'ESERCIZIO 4.2 ho modificato il codice affinché il cristallo occupi solo metà del box di simulazione in ogni dimensione
...
void System :: measure(){ // Measure properties
  ...
  if(_measure_pofv){
    double v;
    for(int i=0; i<_npart; i++){
      v = sqrt( dot( _particle(i).getvelocity() , _particle(i).getvelocity()) ); // calcolo il modulo quadro della velocità
      bin = int(v/_bin_size_v); // capisco in quale bin rientra la velocità
      if(bin < _n_bins_v) _measurement(_index_pofv + bin) += 1.0; // incremento il contatore del bin
    }
    for(int i=0; i<_n_bins_v; i++){
      _measurement(_index_pofv + i) /= (double)_npart*_bin_size_v; // normalizzazione
    }
  }
